...
account key is stored in the application.  YDB is used as the production
multi-instance state backend; DuckDB remains the deterministic local/reference
backend for tests and developer runs.
"""
...
class YdbForecastJournal:
    ...
    def append(self, record: ForecastRecord) -> JournalAppendResult:
        ...
        try:
            ...
            # INSERT can lose a race to another instance. Re-read the durable
            # primary key: an identical winner is idempotent; differing bytes
            # are an immutable identity collision. Any unrelated failure is
            # re-raised when no durable row appeared.
    ...
    def _duplicate_or_collision(self, forecast_id: str, digest: str, existing: str) -> JournalAppendResult:
        if existing != digest:
            raise ForecastCollisionError(
                f"forecast_id collision for {forecast_id}: immutable payload differs"
            )
    ...
    def get(self, forecast_id: str) -> ForecastRecord | None:
        ...
        if row is None:
            return None
        return _forecast_from_dict(json.loads(str(_row_value(row, "payload_json"))))

    def list_recent(self, *, limit: int = 20, symbol: str | None = None) -> list[ForecastRecord]:
        if limit <= 0 or limit > 500:
            raise ValueError("limit must be between 1 and 500")
        # T0 is an ISO timestamp, so descending lexical order is chronological.
        if symbol:
            query = f"""
            DECLARE $symbol AS Utf8;
        ...
        return [
            _forecast_from_dict(json.loads(str(_row_value(row, "payload_json"))))
    ...
    def close(self) -> None:
        # Runtime owns the shared YDB driver/pool.
        return None
...
class YdbOutcomeJournal:
    ...
    def canonical_payload(record: HorizonOutcome) -> tuple[str, str]:
        payload = json.dumps(
            record.to_dict(), sort_keys=True, separators=(",", ":"), ensure_ascii=False, allow_nan=False
        )
        return payload, hashlib.sha256(payload.encode("utf-8")).hexdigest()
    ...
    def list_for_forecast(self, forecast_id: str) -> list[HorizonOutcome]:
        ...
        return [
            _outcome_from_dict(json.loads(str(_row_value(row, "payload_json"))))
            for row in _rows(result)
        ]
