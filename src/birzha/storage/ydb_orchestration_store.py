...
class YdbOrchestrationStore(OrchestrationStore):
    ...
    def _init_schema(self) -> None:
        self._pool.execute_with_retries(
            ...
            CREATE TABLE IF NOT EXISTS `{self._runs_table}` (
                workflow_id Utf8 NOT NULL,
                kind Utf8 NOT NULL,
                stage Utf8 NOT NULL,
                status Utf8 NOT NULL,
                created_at Utf8 NOT NULL,
                updated_at Utf8 NOT NULL,
                metadata_json Utf8 NOT NULL,
                last_error Utf8 NOT NULL,
        ...
        self._pool.execute_with_retries(
            ...
            CREATE TABLE IF NOT EXISTS `{self._actions_table}` (
                ...
                status Utf8 NOT NULL,
                attempt Uint64 NOT NULL,
                max_attempts Uint64 NOT NULL,
                lease_owner Utf8 NOT NULL,
                lease_until Utf8 NOT NULL,
                evidence_json Utf8 NOT NULL,
                last_error Utf8 NOT NULL,
    ...
    def get_workflow(self, workflow_id: str) -> WorkflowRun | None:
        result = self._pool.execute_with_retries(
            f"""
            DECLARE $workflow_id AS Utf8;
            SELECT * FROM `{self._runs_table}` WHERE workflow_id = $workflow_id;
            """,
    ...
    def list_actions(self, workflow_id: str, *, stage: WorkflowStage | None = None) -> list[WorkflowAction]:
        if stage is None:
            ...
            DECLARE $stage AS Utf8;
            SELECT * FROM `{self._actions_table}`
            WHERE workflow_id = $workflow_id AND stage = $stage
            ORDER BY sequence, action_id;
            """
    ...
    def claim_next(
        ...
        *,
        ...
        self._pool.execute_with_retries(
            ...
            """,
            {
                "$action_id": _utf8(candidate.action_id),
                "$now": _utf8(now),
                "$worker_id": _utf8(worker_id),
                "$lease_until": _utf8(lease_until),
                "$attempt": _uint64(candidate.attempt + 1),
            },
            retry_settings=ydb.RetrySettings(idempotent=True),
        )
    ...
    def reset_action(self, action_id: str) -> WorkflowAction:
        ...
        self._pool.execute_with_retries(
            ...
            WHERE action_id = $action_id AND status = 'FAILED';
            """,
            {"$action_id": _utf8(action_id)},
            retry_settings=ydb.RetrySettings(idempotent=True),
        )
    ...
    def _get_action(self, action_id: str) -> WorkflowAction | None:
        result = self._pool.execute_with_retries(
            f"""
            DECLARE $action_id AS Utf8;
            SELECT * FROM `{self._actions_table}` WHERE action_id = $action_id;
            """,
            {"$action_id": _utf8(action_id)},
            retry_settings=ydb.RetrySettings(idempotent=True),
        )
...
def _workflow_from_row(row: object) -> WorkflowRun:
    ...
    return WorkflowRun(
        workflow_id=str(_value(row, "workflow_id")),
        kind=str(_value(row, "kind")),
        stage=WorkflowStage(str(_value(row, "stage"))),
        status=WorkflowStatus(str(_value(row, "status"))),
        created_at=str(_value(row, "created_at")),
        updated_at=str(_value(row, "updated_at")),
        metadata=json.loads(str(_value(row, "metadata_json")) or "{}"),
        last_error=last_error or None,
    )


def _action_from_row(row: object) -> WorkflowAction:
    lease_owner = str(_value(row, "lease_owner"))
    lease_until = str(_value(row, "lease_until"))
    last_error = str(_value(row, "last_error"))
    return WorkflowAction(
        action_id=str(_value(row, "action_id")),
        workflow_id=str(_value(row, "workflow_id")),
        stage=WorkflowStage(str(_value(row, "stage"))),
        kind=str(_value(row, "kind")),
        sequence=int(_value(row, "sequence")),
        payload=json.loads(str(_value(row, "payload_json")) or "{}"),
        status=ActionStatus(str(_value(row, "status"))),
        attempt=int(_value(row, "attempt")),
        max_attempts=int(_value(row, "max_attempts")),
        lease_owner=lease_owner or None,
        lease_until=lease_until or None,
        evidence=json.loads(str(_value(row, "evidence_json")) or "{}"),
        last_error=last_error or None,
    )
