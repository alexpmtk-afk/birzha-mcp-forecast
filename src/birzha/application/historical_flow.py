...
class HistoricalFlowDataService:
    ...
    def tradestats(self, instrument: Instrument, *, from_date: str, till_date: str) -> list[dict[str, object]]:
        dataset="TRADESTATS"
        key=instrument.secid
        if not self.store.is_verified(dataset,key,from_date,till_date):
            rows=self.analytics.fetch_tradestats(instrument,from_date=from_date,till_date=till_date)
    ...
    def futoi(self, instrument: Instrument, *, from_date: str, till_date: str) -> list[dict[str, object]]:
        dataset="FUTOI"
        key=(instrument.root_symbol or instrument.symbol).strip()
        if instrument.asset_class != "future":
            return []
        if not self.store.is_verified(dataset,key,from_date,till_date):
            rows=self.analytics.fetch_futoi(instrument,from_date=from_date,till_date=till_date)
            self.store.upsert_rows(dataset,key,rows,"MOEX_FUTOI")
            self.store.mark_verified(dataset,key,from_date,till_date)
        return self.store.read_rows(dataset,key,from_date,till_date)

    def sync(self, symbol: str, *, from_date: str, till_date: str) -> dict[str, object]:
        start=date.fromisoformat(from_date[:10]); finish=date.fromisoformat(till_date[:10])
        if start > finish:
        ...
        for instrument,left,right in segments:
            trade_count += len(self.tradestats(instrument, from_date=left.isoformat(), till_date=right.isoformat()))
        futoi_count=0
        future=next((item[0] for item in reversed(segments) if item[0].asset_class == "future"),None)
        if future is not None:
            futoi_count=len(self.futoi(future,from_date=start.isoformat(),till_date=finish.isoformat()))
    ...
    def _segments(self, symbol: str, start: date, finish: date):
        ...
        if not timeline:
            return ((self.market_data.resolve(symbol,as_of=finish),start,finish),)
        result=[]; current=timeline[0][1]; left=timeline[0][0]; right=left
        for day,instrument in timeline[1:]:
            if instrument.secid == current.secid:
                right=day; continue
            result.append((current,left,right)); current=instrument; left=day; right=day
