...
class FakePool:
    ...
    def execute_with_retries(self, query, parameters=None, **kwargs):
        ...
        if compact.startswith("DECLARE") and "INSERT INTO" in compact:
            item = {
                "holdout_start": parameters["$holdout_start"][0],
                "holdout_end": parameters["$holdout_end"][0],
                "protocol": parameters["$protocol"][0],
                "model_fingerprint": parameters["$model_fingerprint"][0],
                "consumed_at": parameters["$consumed_at"][0],
            }
            if any(
                existing["holdout_start"] == item["holdout_start"]
                and existing["holdout_end"] == item["holdout_end"]
...
def test_first_holdout_claim_succeeds_and_is_persisted() -> None:
    pool = FakePool()
