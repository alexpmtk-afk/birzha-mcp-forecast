...
def executor_for(
    policy: UpstreamPolicy, clock: FakeClock, *, distributed: bool = False
) -> SafeRequestExecutor:
    ...
    return SafeRequestExecutor(
        policy, gate=gate, sleeper=clock.sleep, jitter=lambda: 0.0
    )
...
def test_moex_default_policies_reserve_ten_percent_headroom() -> None:
    ...
    assert MOEX_ISS_PUBLIC_POLICY.effective_min_interval_seconds == pytest.approx(
        0.5 / 0.9
    )
    ...
    assert MOEX_AUTHENTICATED_POLICY.effective_min_interval_seconds == pytest.approx(
        1.0 / 0.9
    )
...
def test_large_command_uses_nominal_attempt_window_without_double_reserving_retries() -> None:
    clock = FakeClock()
    policy = MOEX_ISS_PUBLIC_POLICY
    executor = executor_for(policy, clock)
    governor = UpstreamRequestGovernor(
        policy, executor, clock=clock, sleeper=clock.sleep
    )
    plan = governor.plan(20)
    assert plan.soft_attempts_per_window == 18
    assert plan.max_logical_requests_per_batch == 18
    assert [batch.size for batch in plan.batches] == [18, 2]
    assert all(
        batch.worst_case_attempts <= policy.max_requests_per_operation
        for batch in plan.batches
    )


def test_retries_still_stretch_window_and_respect_attempt_pacing() -> None:
    clock = FakeClock()
    ...
    def request_for_task(task: int):
        def request() -> FakeResponse:
            attempts[task] = attempts.get(task, 0) + 1
            return FakeResponse(503 if attempts[task] == 1 else 200)

        return request

    results = governor.execute(list(range(9)), request_for_task)
    assert len(results) == 9
    assert all(result.status_code == 200 for result in results)
    assert executor.total_requests_used == 18
    assert clock.now == pytest.approx(
        17 * policy.effective_min_interval_seconds
    )
...
def test_large_command_continues_automatically_with_window_waits() -> None:
    ...
    results = governor.execute(
        list(range(20)), lambda _: (lambda: FakeResponse(200))
    )
...
def test_remote_multi_instance_mode_fails_closed_without_distributed_gate() -> None:
    ...
    with pytest.raises(UnsafeUpstreamConfiguration):
        control.governor(
            "moex-public", policy, require_distributed_gate=True
        )
...
def test_remote_mode_accepts_distributed_gate_contract() -> None:
    ...
    control = ProcessUpstreamControlPlane(
        gate_factory=lambda _provider_key: FakeDistributedGate(
...
def test_control_plane_level_distributed_requirement_fails_closed() -> None:
    clock = FakeClock()
    control = ProcessUpstreamControlPlane(
        require_distributed_gate=True, clock=clock, sleeper=clock.sleep
    )
    with pytest.raises(UnsafeUpstreamConfiguration):
